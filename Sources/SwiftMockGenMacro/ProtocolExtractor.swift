...
/// a `GeneratMockMacroError.onlyApplicableToProtocol` error.
struct Extractor {
    func extractProtocolDecl(from declaration: DeclSyntaxProtocol) throws -> ProtocolDeclSyntax {
        guard let protocolDeclaration = declaration.as(ProtocolDeclSyntax.self) else {
            throw GeneratMockMacroError.onlyApplicableToProtocol
        }
